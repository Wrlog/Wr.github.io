...
class Dog:
def **init**(self, name, breed): # We make 'name' private
self.\_\_name = name
self.breed = breed
...
print("The dog's name is:", dog1.get_name()) 2. Inheritance
...
class Dog(Animal):
def **init**(self, name, age, breed): # Call the **init** of the parent class
super().**init**(name, age)
...
class Cat(Animal): # This class inherits speak() and show_info() # without any changes.
pass
...
print("\n--- Animal sounds ---")
for animal in animals: # This is polymorphism in action!
animal.speak()
Output:
...
# If you try to create a class that _forgets_...
