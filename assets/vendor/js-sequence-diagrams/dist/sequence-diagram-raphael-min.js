...
!(function () {
  ...
  function wobble (x1, y1, x2, y2) {
    ...
    const factor =
      Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)) / 25
  ...
  if (
    ...
    const LINE = { stroke: '#000000', 'stroke-width': 2, fill: 'none' }
    const RECT = { stroke: '#000000', 'stroke-width': 2, fill: '#fff' }
  ...
  if (
    ...
    typeof Raphael === 'undefined' && typeof Snap === 'undefined')
  ) {
  ...
  (Diagram.prototype.drawSVG = function (container, options) {
    const defaultOptions = { theme: 'hand' }
    if (
