...
class PortfolioPageEnhancer {
    ...
    private setupFiltering(): void {
        ...
        if (filterContainer) {
            document.querySelector('.portfolio-section .markdown-body')?.insertBefore(
                filterContainer,
                document.querySelector('.portfolio-section .markdown-body > h2:first-of-type')
            );
        }
    }
    ...
    private createFilterContainer(): HTMLElement | null {
        ...
        filters.forEach(filter => {
            ...
            button.addEventListener('click', () => this.filterProjects(filter.toLowerCase()));
